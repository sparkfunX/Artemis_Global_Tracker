# Artemis Global Tracker: Convert CSV DateTime to KML
...
# This code converts a stitched .csv file into .kml files for GoogleEarth

# The .csv file should have been processed by Artemis_Global_Tracker_CSV_DateTime.py
...
# This code expects the .csv file to contain:
...
# Column 8 = message MOMSN (appended by Artemis_Global_Tracker_Stitcher.py)

# You can use the Artemis_Global_Tracker_Configuration_Tool to configure the Tracker
# to send the appropriate MOFIELDS:
#
# DATETIME
# LAT
# LON
...
print('Artemis Global Tracker: DateTime CSV to KML Converter')
print
    ...
    for line in reader:
        if (line[0][:2] == 'RB') : # Does the message payload have an RB prefix?
            try:
                ...
                momsn = line[8] # Extract the MOMSN
            except:
                latitude = 0.
                longitude = 0.
        else:
            try:
                ...
                momsn = line[7] # Extract the MOMSN
            except:
                latitude = 0.
                longitude = 0.
        ...
        if (latitude == 0.) and (longitude == 0.): # Check lat and lon are valid
            ...
            # Update point kml
            pnt = point_kml.newpoint(name=momsn)
            pnt.coords=[(longitude,latitude,altitude)]
            pnt.style = style
            # Update arrow kml
            pnt = arrow_kml.newpoint(name=momsn)
            pnt.coords=[(longitude,latitude,altitude)]
